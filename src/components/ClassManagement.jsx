...
export default function ClassManagement({ userRole, userName, setActiveTab }) {
    ...
    const getComputedStatus = (cls) => {
        ...
        const earliestSession = cls.sessions.reduce(
        ...
        const fetchClasses = async () => {
            try {
                ...
                const mapped = data.map((backendCls) => {
                    const classSessions = (Array.isArray(sessionsData) ? sessionsData : []).filter(
                        s => s.classId === backendCls._id || (s.classId && s.classId._id === backendCls._id)
                    ).map(s => ({
                        ...s,
                    ...
                    return {
                        ...
                        status: backendCls.status || 'Active',
                        trainer: backendCls.assignedTrainer?.name || 'Unassigned',
    ...
    // Keep auto-status effect disabled for now since we rely on backend status mostly
    // useEffect(() => { ... });
    ...
    const handleDownloadCSV = (batchId, tab, batchStudents, sessionColumns, allActivities) => {
        ...
        if (tab === 'Attendance') {
            const headers = [
                'Student Name',
                'Attendance %',
                ...sessionColumns.map((col, i) => `S${i + 1} (${col.subLabel.replace(/,/g, '')})`),
            ];
            ...
            batchStudents.forEach((student) => {
                const row = [
                    ...
                    `${student.attendance}%`,
                    ...sessionColumns.map(
                        (col, idx) => student.detailedReport?.sessions?.[idx]?.attendance || '-',
                    ),
                ];
                csvContent += row.join(',') + '\n';
            });
            ...
            const headers = [
                'Student Name',
                'Avg Mark',
                ...allActivities.map((a) => `"${a.replace(/"/g, '""')}"`),
            ];
            csvContent += headers.join(',') + '\n';
            ...
            batchStudents.forEach((student) => {
                ...
                const avg =
                    allActivities.length > 0 ? (total / allActivities.length).toFixed(1) : '0.0';
    ...
    const getPendingFeedbackStudents = (batchId, trainerName) => {
        ...
        const manualData = [
            {
                ...
                feedback: 'Amazing depth of knowledge in Python.',
                rating: 5,
            },
            {
                id: 'm2',
                ...
                role: 'Co-Trainer',
                studentName: 'Alice Johnson',
                feedback: 'Very helpful during the hands-on lab.',
                rating: 4,
            },
            {
                id: 'm3',
                ...
                date: '2026-05-15',
                time: '08:00 - 10:00',
                staffName: 'Margaret Hamilton',
                role: 'Trainer',
                studentName: 'Zara Ali',
                feedback: 'Flawless execution of the session.',
                rating: 5,
            },
            {
                id: 'm4',
                ...
                feedback: 'Good pacing, but complex concepts need more time.',
                rating: 3,
            },
            {
                id: 'm5',
                ...
                feedback: 'Excellent session on Data Structures.',
                rating: 5,
            },
            {
                id: 'm6',
                ...
                role: 'Trainer',
                studentName: 'Emily Davis',
                feedback: 'Very engaging and interactive.',
                rating: 4,
            },
        ];
        ...
        if (cls) {
            cls.sessions?.forEach((session, sIdx) => {
                batchStudents.forEach((student) => {
                    const hash =
                        (student.id * 1000 + sIdx + new Date(session.date).getTime()) % 100;
                    if (hash > 70) {
    ...
    const updateClassOnServer = async (classObj, field, valueName) => {
        let payload = {};
        if (field === 'assignedTrainer') {
             if (valueName === 'Unassigned') {
                 alert('Warning: Backend constraints currently prevent unassigning Trainers. Please select a valid Trainer.');
                 return false;
             }
             const u = users.find(x => x.name === valueName);
             if (u) payload.assignedTrainer = u._id;
        }
        else if (field === 'coTrainers') {
            if (valueName === 'Unassigned' || !valueName) payload.coTrainers = [];
            else {
                const names = typeof valueName === 'string' ? valueName.split(',').map(s => s.trim()) : valueName;
                const ids = names.map(n => users.find(x => x.name === n)?._id).filter(Boolean);
                // Deduplicate co-trainer ObjectIds
                payload.coTrainers = [...new Set(ids)];
            }
        }
        else if (field === 'assignedLab') {
             if (valueName === 'Unassigned') {
                 alert('Warning: Backend constraints currently prevent unassigning Labs. Please select a valid Lab.');
                 return false;
             }
             const l = labs.find(x => x.name === valueName || (x.name && x.name.includes(valueName)));
             if (l) payload.assignedLab = l._id;
        }
        
        if (Object.keys(payload).length === 0) {
            alert('Error: Could not find the selected assignment in the database. Update aborted.');
            return false;
        }

        try {
            const res = await fetch(`${baseUrl}/api/classes/${classObj._id}`, {
                 method: 'PUT',
            ...
            console.error('Failed to update class on server:', e);
            alert('Failed to update assignment on the server. Please try again.');
            return false;
        }
    };

    const handleModalSelect = async (value) => {
        if (activeModal === 'Transfer-Tabs') {
            ...
            if (targetClass) {
                ...
                const sessionToEdit = updatedSessions[activeSessionIdx];
                if (sessionToEdit && sessionToEdit._id) {
                    const u = users.find(x => x.name === value.staff);
                    if (u) {
                        try {
                            const payload = value.type === 'Trainer' 
                                ? { transferredTo: u._id }
                                : { transferredCoTrainerTo: u._id };
                                
                            const res = await fetch(`${baseUrl}/api/sessions/${sessionToEdit._id}`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                                body: JSON.stringify(payload)
                            });
                            ...
                            if (res.ok) {
                                ...
                                updatedSessions[activeSessionIdx] = {
                                    ...
                                    transferredTo: updated.transferredTo?.name || updated.transferredTo,
                                    transferredCoTrainerTo: updated.transferredCoTrainerTo?.name || updated.transferredCoTrainerTo
                                };
                            ...
                            console.error(e);
                            alert('Failed to transfer session');
                        }
                    }
                }
            }
        } else if (activeModal === 'Trainer' || activeModal === 'Co-Trainer') {
            const type = value?.type || activeModal;
            const staffValue = value?.staff || value;
            const field = type === 'Trainer' ? 'assignedTrainer' : 'coTrainers';
            const stateField = type === 'Trainer' ? 'trainer' : 'coTrainers';
            
            const currentClassObj = classes.find((c) => c.id === activeClassId);
            if (currentClassObj) {
                const success = await updateClassOnServer(currentClassObj, field, staffValue);
                if (success) updateClass(activeClassId, stateField, staffValue);
            }
        } else if (activeModal === 'Lab') {
            const currentClassObj = classes.find((c) => c.id === activeClassId);
            if (currentClassObj) {
                const success = await updateClassOnServer(currentClassObj, 'assignedLab', value);
                if (success) updateClass(activeClassId, 'lab', value);
            }
        }
    ...
    const handleAddSession = async (classId) => {
        ...
        if (targetClass) {
            ...
            if (editingSessionIdx !== null) {
                ...
                if (slotType === 'morning') slot = { startTime: '09:00', endTime: '12:00' };
                    ...
                    slot = { startTime: customSlots[0].startTime, endTime: customSlots[0].endTime };

                if (slot && slot.startTime && slot.endTime) {
                    const sessionToEdit = updatedSessions[editingSessionIdx];
                    if (sessionToEdit._id) {
                        try {
                            const res = await fetch(`${baseUrl}/api/sessions/${sessionToEdit._id}`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                                body: JSON.stringify({ date: newDate, startTime: slot.startTime, endTime: slot.endTime })
                            });
                ...
                selectedSlots.forEach((slotType) => {
                    if (slotType === 'morning') {
                        newSlots.push({ startTime: '09:00', endTime: '12:00' });
                    } else if (slotType === 'afternoon') {
                        newSlots.push({ startTime: '13:00', endTime: '16:00' });
                    } else if (slotType === 'custom') {
                        customSlots.forEach((cs) => {
                            if (cs.startTime && cs.endTime) newSlots.push(cs);
                        });
                    }
                });
                ...
                for (const slot of newSlots) {
                    try {
                        const res = await fetch(`${baseUrl}/api/sessions`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                            body: JSON.stringify({ classId: targetClass._id, date: newDate, startTime: slot.startTime, endTime: slot.endTime })
                        });
                        if (res.ok) {
    ...
    const handleRemoveSession = async (classId, sessionIndex) => {
        if (!window.confirm('Are you sure you want to delete this session?')) return;
        
        const targetClass = classes.find((c) => c.id === classId);
        if (targetClass) {
            const sessionToRemove = targetClass.sessions[sessionIndex];
            if (sessionToRemove && sessionToRemove._id) {
                try {
                    const res = await fetch(`${baseUrl}/api/sessions/${sessionToRemove._id}`, {
                        method: 'DELETE',
                        headers: { Authorization: `Bearer ${token}` }
                    });
                    if (res.ok) {
    ...
    if (loading) {
        return <div className="p-8 text-center text-gray-500 font-bold">Loading classes...</div>;
    }

    const filteredClasses = classes.filter((cls) => {
        const matchStatus = filters[cls.status];
        const matchTrainer =
            userRole === 'Trainer' || userRole === 'Co-Trainer'
                ? cls.trainer === userName || cls.coTrainers?.includes(userName)
                : true;
        ...
        <>
            <div className="p-6 animate-fade-in">
                ...
                <div
                    ...
                    style={{ gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '1.5rem' }}
                >
                    {filteredClasses.length === 0 ? (
                        <div
                            style={{ gridColumn: '1 / -1', padding: '60px 0', textAlign: 'center' }}
                        >
                            <div
                                style={{
                                    display: 'inline-flex',
                            ...
                            <p style={{ color: '#64748B', maxWidth: '400px', margin: '0 auto' }}>
                                There are currently no classes available in the database.
                                {canCreateClass &&
                                    " Click the 'Create Class' button above to add a new batch."}
                            </p>
                        </div>
                    ) : (
                                ...
                                >
                                    ...
                                    {isAdmin ? (
                                        ...
                                        <select
                                            value={cls.status}
                                            onChange={(e) =>
                                                updateClass(cls.id, 'status', e.target.value)
                                            }
                                            style={{
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span style={{ fontWeight: 600 }}>
                                                Number of Sessions Added: {cls.sessions.length}
                                            </span>
                                        </div>
                                        {cls.sessions.map((session, idx) => {
                                            const hasTransfer = !!(
                                                session.transferredFrom ||
                                                session.transferredCoTrainerFrom ||
                                                session.transferredTo ||
                                                session.transferredCoTrainerTo
                                            );
                                            return (
                                                <div
                                                    key={idx}
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        {canCreateClass && (
                                                            <button
                                                                onClick={() =>
                                                                    handleRemoveSession(cls.id, idx)
                                                                }
                                                                style={{
                                                                    background: 'none',
                                                                    border: 'none',
                                    ...
                                    {canCreateClass && (
                                        <div style={{ marginTop: '4px' }}>
                                            {showDatePickerFor === cls.id ? (
                                                            ...
                                                            >
                                                                ...
                                                                <div
                                                                    style={{
                                                                        ...
                                                                        height: '16px',
                                                                        borderRadius: '50%',
                                                                        border: `2px solid ${selectedSlots.includes('morning') ? '#3B82F6' : '#D1D5DB'}`,
                                                                        backgroundColor:
                                                                            selectedSlots.includes(
                                                                                'morning',
                                                                            )
                                                            ...
                                                            >
                                                                ...
                                                                <div
                                                                    style={{
                                                                        ...
                                                                        height: '16px',
                                                                        borderRadius: '50%',
                                                                        border: `2px solid ${selectedSlots.includes('afternoon') ? '#3B82F6' : '#D1D5DB'}`,
                                                                        backgroundColor:
                                                                            selectedSlots.includes(
                                                                                'afternoon',
                                                                            )
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div
                                                                            style={{
                                                                                ...
                                                                                height: '16px',
                                                                                borderRadius: '50%',
                                                                                border: `2px solid ${selectedSlots.includes('custom') ? '#3B82F6' : '#D1D5DB'}`,
                                                                                backgroundColor:
                                                                                    selectedSlots.includes(
                                                                                        'custom',
                                                                                    )
                                                ...
                                                <button
                                                    onClick={() => setShowDatePickerFor(cls.id)}
                                                    style={{
                                                        background: 'none',
                                    ...
                                    <div
                                        style={{
                                            ...
                                            border: '1px solid #E5E7EB',
                                            borderRadius: '12px',
                                            cursor: !canCreateClass ? 'default' : 'pointer',
                                            backgroundColor: '#F8FAFC',
                                            gap: '8px',
                                            minWidth: 0,
                                        }}
                                        onClick={() => canCreateClass && handleOpenModal('Lab', cls.id)}
                                    >
                                        <div
                                            style={{
                                                display: 'flex',
                                    ...
                                    <div
                                        ...
                                        onClick={() =>
                                            canCreateClass && handleOpenModal('Trainer', cls.id)
                                        }
                                    >
                                        <div
                                            style={{
                                    ...
                                    <div
                                        ...
                                        onClick={() =>
                                            canCreateClass && handleOpenModal('Co-Trainer', cls.id)
                                        }
                                    >
                                        <div
                                            style={{
            ...
            <StaffSelectionModal
                isOpen={
                    activeModal === 'Trainer' ||
                    activeModal === 'Co-Trainer' ||
                    activeModal === 'Transfer-Tabs'
                }
                    ...
                    >
                        <div
                            ...
                            style={{
                                backgroundColor: 'white',
                                boxShadow:
                                    '0 30px 60px -15px rgba(15, 23, 42, 0.25), 0 0 0 1px rgba(15, 23, 42, 0.05)',
                                width: '420px',
                                maxHeight: '90vh',
                                overflowY: 'auto',
                                padding: '40px 32px',
                        ...
                        >
                            ...
                            >
                                Details regarding the <strong>{transferInfo.type}</strong>{' '}
                            ...
                            <button
                                ...
                                onMouseEnter={(e) => {
                                    ...
                                    e.currentTarget.style.boxShadow =
                                        '0 12px 20px -4px rgba(15, 23, 42, 0.3)';
                                }}
                                onMouseLeave={(e) => {
                                    e.currentTarget.style.backgroundColor = '#1E293B';
                                    e.currentTarget.style.transform = 'translateY(0)';
                                    e.currentTarget.style.boxShadow =
                                        '0 8px 16px -4px rgba(30, 41, 59, 0.25)';
                                }}
                            >
                                Close Details
                            </button>
            ...
            {showReportFor &&
                createPortal(
                    (() => {
                        const batchStudents = users.filter(
                            (u) =>
                                u.role === 'Student' && u.batch?.trim() === showReportFor?.trim(),
                        );

                        // Tab 1: Attendance dynamic columns
                        const maxSessions = Math.max(
                            ...batchStudents.map((s) => s.detailedReport?.sessions?.length || 0),
                            0,
                        );
                        ...
                        const sessionColumns = Array.from({ length: maxSessions }, (_, i) => {
                            ...
                            const date =
                                classSession?.date ||
                                batchStudents.find((s) => s.detailedReport?.sessions?.[i])
                                    ?.detailedReport?.sessions?.[i]?.date ||
                                '';
                            const time = classSession
                        ...
                        const allActivities = Array.from(
                            new Set(
                                batchStudents.flatMap(
                                    (s) =>
                                        s.detailedReport?.performance?.map((p) => p.activity) || [],
                                ),
                            ),
                        );
                                    ...
                                    <div className="flex gap-6 border-b border-gray-200 mb-6">
                                        ...
                                        <div
                                            ...
                                            style={{
                                                ...
                                                fontWeight: 600,
                                                borderBottom:
                                                    activeReportTab === 'Activity Report'
                                                        ? '2px solid var(--color-primary)'
                                                        : '2px solid transparent',
                                                color:
                                                    activeReportTab === 'Activity Report'
                                        ...
                                        <div className="table-container mb-4">
                                            ...
                                            >
                                                ...
                                                <tbody>
                                                    {batchStudents.map((student) => {
                                                        const performance =
                                                            student.detailedReport?.performance ||
                                                            [];
                                                        let total = 0;
                                                        allActivities.forEach((act) => {
                                                            const record = performance.find(
                                                                (p) => p.activity === act,
                                                            );
                                                            if (record) {
                                                                const val =
                                                                    parseInt(
                                                                        record.score.split('/')[0],
                                                                    ) || 0;
                                                                total += val;
                                                            }
                                                        });
                                        ...
                                        >
                                            {tasks.length === 0 ? (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        No tasks assigned for your role.
                                                    </p>
                                                </div>
                                            ) : (
                                                tasks.map((task) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            Batch: {cls.id} • {pendingList.length} Students Pending
                                        </p>
                                    </div>
                                        ...
                                        >
                                            {pendingList.length === 0 ? (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        All students in this batch have submitted
                                                        their feedback.
                                                    </p>
                                                </div>
                                            ) : (
